...
**Status.** `Complete` — **RESOLVED 2026-08-31**, no longer merely parked. **Evidence:** PSG 2025 p.84 prints the 59E profile as `5 15/16"` with `(5.93")` beside it — the page reference the park was waiting for. `interlake-2026-09` carries 5.93 on all 42 rows; 5.92 is superseded (it appears nowhere in the source) and 5.928 with it. Note 5 15/16 = 5.9375 rounds to **5.94**, so the printed 5.93 is the manufacturer's truncation, not a rounding — both wrong values are pinned by test and both breaks proven red. `interlake-2026-08` deliberately still asserts 5.92, because a frozen release must keep reporting what it transcribed. **Superseded by `P0-009`:** both this and the 65E/65Q discrepancy are closed by storing the exact published fraction (59E = 5.9375, 65E/65Q = 6.5625) and rounding only for display. Every figure that had been argued over was an approximation of a fraction the guide states exactly.
